...
app=graph.compile(checkpointer=memory,interrupt_before=["assess"])
...
if __name__ == "__main__":
    ...
    print("paused before assessment. current state: ")
    print(result["extracted_info"])

    approval=input("\nApprove this candidate for assessment (yes or no) :")

    if approval.lower()=="yes":
        final_result=app.invoke(None,config=config)
        print("\nFinal assessment: \n")
        print(final_result["final_assessment"])
    else:
        print("\nAssessment skipped by reviewer.")
    
    config2={"configurable":{"thread_id":"candidate_002"}}
    
    print("Running extraction and verification...")
    for event in app.stream({
        "candidate_name": "Aashly",
        "resume_text":resume_text,
        "job_requirements":job_requirements,
        "extracted_info":"",
        "final_assessment":""
    ...
    print("\nGraph paused before assessment")

    current_state=app.get_state(config2)
    print(f"\nCurrent extracted info")
    print(current_state.values["extracted_info"])

    approve=input("Approve for assessmen(yes or no): ")

    if approve.lower()=="yes":
        print("\nResuming...")
        for event in app.stream(None,config=config2):
            print("---")
            print(event)
    else:
        print("\nApproval denied for assessment")
